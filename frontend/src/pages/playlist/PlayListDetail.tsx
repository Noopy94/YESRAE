...
export default function PlayListDetail() {
  ...
  const [currentPlayListDetail, setCurrentPlayListDetail] = useState<playList>({
    ...
    userId:0,
    nickname:'',
    songs:[{songId:'',songTitle:'',songUrl:'',songArtist:'',songImgUrl:''}],
  })
  ...
  const registComment = () => {
    registPlaylistComment(Number(playListId), user.userId, comment).then((res) => {
      if (res) {
        alert('댓글이 등록되었습니다.');
        if(currentPlayListDetail.userId !== user.userId){
          registNotification('playlistComment', currentPlayListDetail.title, user.nickname, currentPlayListDetail.userId);
        }
        history.go((0));
      }
    });
        ...
        <main className='w-10/12 px-20 pt-12'>
          <div className='flex pb-10 border-b border-gray-900'>
            ...
            <div className='relative px-6'>
              ...
              <div className='pt-3'>
                {currentPlayListDetail.nickname}
              </div>
          ...
          {songsLoading?<SongListComponent songs={currentPlayListDetail.songs} />:<div/>}
          ...
          <textarea className='text-black w-10/12 h-20 inline' placeholder='댓글을 입력해주세요'
